...
from models.utils.args import str2bool
from models.utils.args import parse_args
from models.utils.logging import getLogger
from models.utils.misc import tensor2list
from models.inputters.dataset import PostResponseDataset
from models.inputters.dataloader import DataLoader
from models.models.mmpms import MMPMS
from models.modules.generator import BeamSearch
from models.engine import Engine
from models.engine import evaluate
from models.engine import infer
...
data_arg.add_argument("--data_dir", type=str, default="./data/ES_newdata/")
...
data_arg.add_argument("--min_len", type=int, default=5)
data_arg.add_argument("--max_len", type=int, default=300)

# Model
model_arg = parser.add_argument_group("Model")
model_arg.add_argument("--embed_dim", type=int, default=200)
model_arg.add_argument("--hidden_dim", type=int, default=1024)  #1024
model_arg.add_argument("--num_mappings", type=int, default=20)  #20
...
model_arg.add_argument("--attn_mode", type=str, default='mlp', choices=['none', 'mlp', 'dot', 'general'])
model_arg.add_argument("--use_pretrained_embedding", type=str2bool, default=True)
model_arg.add_argument("--embed_init_scale", type=float, default=0.03)
model_arg.add_argument("--dropout", type=float, default=0.3)
#model_arg.add_argument("--mapping_choose", type=str2bool, default=True)
model_arg.add_argument("--fLoss_mode", type=str, default='fLoss_pre', choices=['no','fLoss_post', 'fLoss_pre','cross'])
model_arg.add_argument("--mapping_mode", type=str, default='mapping_mode_1', choices=['mapping_mode_1','mapping_mode_2', 'mapping_mode_no', 'mapping_mode_new'])
...
infer_arg.add_argument("--infer_batch_size", type=int, default=8)
...
def main():
    ...
    args.Data.vocab_file = args.Data.vocab_file or os.path.join( args.Data.data_dir, "vocab.json")
    args.Data.train_file = args.Data.train_file or os.path.join( args.Data.data_dir, "music.train.pkl")
    args.Data.valid_file = args.Data.valid_file or os.path.join( args.Data.data_dir, "music.valid.pkl")
    args.Data.test_file  = args.Data.test_file  or os.path.join( args.Data.data_dir, "music.test.pkl")
    ...
    if args.infer:
        ...
        # Logger Definition
        logger = getLogger(
            os.path.join(args.save_dir, "train.log"), name="models")
        ...
        # Train
        #logger.info("exe.evaluate(valid_loader, is_save=False) ...")
        #exe.evaluate(valid_loader, is_save=False)
        logger.info("Training starts ...")
        for epoch in range(args.num_epochs):
            exe.train_epoch(train_iter=train_loader, valid_iter=valid_loader, debug=args.debug)
